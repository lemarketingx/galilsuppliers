...
import { Search, Upload, Trash2, Star, Download, RotateCcw, Building2, Phone, Mail, MapPin, Tag, FileSpreadsheet } from 'lucide-react';
import logo from './galil-logo.webp';

const STORAGE_KEY = 'galil_suppliers_directory_v4';

function makeId() {
  return typeof crypto !== 'undefined' && crypto.randomUUID ? crypto.randomUUID() : `${Date.now()}_${Math.random().toString(16).slice(2)}`;
}

const demoSuppliers = [
  { id: makeId(), name: 'Technobar', field: 'מכשור / ברזים / צנרת', project: 'דוגמה', contact: 'Adi', phone: '', email: '', city: 'ישראל', catalog: '1001', rating: 4, status: 'מאושר', notes: 'נתוני דוגמה בלבד' },
  { id: makeId(), name: 'ABB', field: 'חשמל / אוטומציה / ציוד תעשייתי', project: 'דוגמה', contact: '', phone: '', email: '', city: '', catalog: '1002', rating: 5, status: 'מאושר', notes: 'נתוני דוגמה בלבד' },
  { id: makeId(), name: 'HAPMAN', field: 'מערכות שינוע / ציוד מכני', project: 'דוגמה', contact: '', phone: '', email: '', city: 'USA', catalog: '1003', rating: 4, status: 'בבדיקה', notes: 'נתוני דוגמה בלבד' },
  { id: makeId(), name: 'DELFIN S.R.L', field: 'מערכות העמסה / ציוד תהליך', project: 'דוגמה', contact: '', phone: '', email: '', city: 'Italy', catalog: '1004', rating: 3, status: 'בבדיקה', notes: 'נתוני דוגמה בלבד' },
];

const engineeringFields = [
  'כל התחומים', 'צנרת', 'חשמל', 'מכשור ובקרה', 'הנדסה אזרחית', 'קונסטרוקציה', 'מכונות', 'ציוד תהליך', 'משאבות', 'ברזים', 'פלדות ומתכות', 'בטון', 'קבלני ביצוע', 'לוחות חשמל', 'כבלים', 'אוטומציה', 'HVAC', 'בטיחות', 'בידוד וצבע', 'שינוע', 'הדרכה', 'תוכנה', 'ייעוץ', 'שונות', 'רכש כללי'
];
...
function supplierFromRow(row, index) {
  ...
  const name = normalize(getByHeaders(row, ['שם ספק/קבלן', 'שם ספק', 'ספק', 'שם הספק', 'Supplier', 'Vendor'])) || `ספק ${index + 1}`;
  const field = normalize(getByHeaders(row, ['תחום עיסוק', 'תחום', 'תחום פעילות', 'קטגוריה', 'דיסציפלינה', 'תאור משאב', 'תיאור משאב', 'Category'])) || 'לא סווג';
  const project = normalize(getByHeaders(row, ['פרויקט', 'Project']));
  const supplierNo = normalize(getByHeaders(row, ["מס' ספק/קבלן", 'מס ספק/קבלן', 'מספר ספק/קבלן', 'מספר ספק', 'קוד ספק', 'Vendor ID', 'Supplier No']));
  const itemDescription = normalize(getByHeaders(row, ['תאור הסעיף/פרק', 'תיאור הסעיף/פרק', 'תאור סעיף', 'תיאור סעיף', 'תיאור', 'תאור', 'Notes', 'Remarks']));
  ...
  return {
    ...
    field,
    project,
    contact: normalize(getByHeaders(row, ['איש קשר', 'איש קשר ספק', 'Contact'])),
    phone: normalize(getByHeaders(row, ['טלפון', 'נייד', 'מספר טלפון', 'Phone', 'Mobile'])),
    email: normalize(getByHeaders(row, ['מייל', 'אימייל', 'דואר אלקטרוני', 'Email'])),
    city: normalize(getByHeaders(row, ['עיר', 'מיקום', 'מדינה', 'כתובת', 'Location', 'City', 'Country'])),
    catalog: supplierNo || normalize(getByHeaders(row, ['מק״ט', 'מקט', 'מק"ט', 'Catalog', 'SKU'])),
    status: normalize(getByHeaders(row, ['סטטוס', 'Status'])) || 'חדש',
    notes: itemDescription,
    rating: Math.min(5, Math.max(0, numberOrDefault(getByHeaders(row, ['דירוג', 'Rating']), 0))),
    raw: row,
  };
}
...
export default function App() {
  ...
  const filtered = useMemo(() => {
    ...
    return suppliers.filter((s) => {
      const text = `${s.name} ${s.field} ${s.project || ''} ${s.contact} ${s.email} ${s.phone} ${s.city} ${s.catalog} ${s.notes}`.toLowerCase();
      const byQuery = !q || text.includes(q);
      const byField = field === 'כל התחומים' || String(s.field).includes(field) || String(field).includes(s.field);
      ...
      <main className="container">
        <section className="toolbar card">
          <div className="uploadBox" onClick={() => fileRef.current?.click()}>
            ...
            <div>
              ...
              <span>פורמט נתמך: פרויקט, מס&apos; ספק/קבלן, שם ספק/קבלן, תאור הסעיף/פרק, תחום עיסוק</span>
            </div>
        ...
        <section className="filters card">
          <div className="searchBox">
            <Search size={20}/>
            <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="חיפוש לפי שם ספק, תחום, פרויקט, מספר ספק או תיאור סעיף..." />
          </div>
  ...
  return (
    <article className="supplier card">
      ...
      <div className="details">
        {supplier.project && <p><FileSpreadsheet size={16}/> {supplier.project}</p>}
